...
concept ClosureSizeProvider = requires(const T c, const A attrs, const F filter, size_t id) {
    { c.ComputeClosureSize(id) } -> std::convertible_to<size_t>;
    { c.ComputeClosureSize(attrs) } -> std::convertible_to<size_t>;
    { c.ComputeFilteredClosureSize(id, filter) } -> std::convertible_to<size_t>;
    { c.ComputeFilteredClosureSize(attrs, filter) } -> std::convertible_to<size_t>;
};
