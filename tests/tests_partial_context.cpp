...
{
    {
        ...
        [](int argc, char** argv) {
            Bitset<uint64_t> set(134);
            set.Add(15); set.Add(16); set.Add(17); set.Add(18); set.Add(19); set.Add(21); set.Add(75); set.Add(111);
            std::ostringstream oss(std::ios::binary);
            set.Serialize(oss);
            auto s = oss.str();
            std::istringstream iss(s, std::ios::binary);
            Bitset<uint64_t> set2(134);
    ...
    },
    {
        ...
        [](int argc, char** argv) {
            Bitset<uint64_t> set({0ull, 0ull, 0xFFFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF});
            std::ostringstream oss(std::ios::binary);
            set.Serialize(oss);
            auto s = oss.str();
            std::istringstream iss(s, std::ios::binary);
            Bitset<uint64_t> set2(256);
            set2.Deserialize(iss);
            return set == set2 ? 0 : 1;
        }
    },
    {
        ...
        [](int argc, char** argv) {
            PartialContext<uint64_t> context(130, 120);
            context.SetIncidence(1, 63); context.SetIncidence(1, 88); context.SetIncidence(1, 111);
            context.SetIncidence(2, 63); context.SetIncidence(63, 76); context.SetIncidence(87, 110);
            context.SetIncidence(6, 68); context.SetIncidence(35, 35); context.SetIncidence(35, 53);
            context.SetIncidence(12, 63); context.SetIncidence(66, 65); context.SetIncidence(1, 10);
            std::ostringstream oss(std::ios::binary);
            context.Serialize(oss);
            auto s = oss.str();
            std::istringstream iss(s, std::ios::binary);
            PartialContext<uint64_t> context2(iss);
    ...
    },
    {
        ...
        [](int argc, char** argv) {
            ...
            b1.Close();
            std::string s(s1.str());
            if ((uint8_t)s[0] != 0xE2 || (uint8_t)s[1] != 0x5B) return 1;
            std::istringstream s2(s, std::ios::binary);
            InputBitstream b2(s2);
            if (b2.ReadBits<3>() != 0x2 || b2.ReadBits<6>() != 0x3C || b2.ReadBits<5>() != 0xD || b2.ReadBits<2>() != 0x1)
                return 2;
            return 0;
        }
    }
